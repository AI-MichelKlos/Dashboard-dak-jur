...
DATA_START='const DATA='; DATA_END=';\nconst C='; API_ROOT='https://api.jobindsats.dk/v3'
AKASSE_NAMES={
...
def api_get(path):
 ...
 if not token: raise RuntimeError('JOBINDSATS_API_TOKEN mangler')
 req=urllib.request.Request(f"{API_ROOT}/{path.lstrip('/')}",headers={'Accept':'application/json','Authorization':f'Bearer {token}','User-Agent':'Danske-A-kasser-jur-dashboard/1.0'})
 try:
  with urllib.request.urlopen(req,timeout=60) as res:return json.loads(res.read().decode('utf-8'))
 except urllib.error.HTTPError as exc:
  detail=exc.read().decode('utf-8',errors='replace');raise RuntimeError(f'Jobindsats returnerede HTTP {exc.code}: {detail[:300]}') from exc
...
def merge_akasse(sec,rows,column):
 ...
 for p,vals in grouped.items():
  for n,v in vals.items(): combined.setdefault(n,{})[p]=v
 sec['labels']=labels; sec['byAkasse']={n:[combined.get(n,{}).get(p) for p in labels] for n in sec['byAkasse']}
...
def refresh_unemployment(d):
 merge_akasse(d['sections']['unemploymentAkasser'],records('data/y25i01?mgroup.*=*&period.M=latest:120&hierarchy._nykom=/&hierarchy._ygrpi09=/&hierarchy._akassebl=*&format=json'),'Antal ledige fuldtidspersoner')

def refresh_longterm(d):
 merge_akasse(d['sections']['longTermUnemployment'],records('data/y25i09?mgroup.*=*&period.M=latest:120&hierarchy._nykom=/&hierarchy._ygrpi09=/&hierarchy._akassebl=*&format=json'),'Antal langtidsledige fuldtidspersoner')
...
def refresh_activation(d):
 ...
 names={'Tilbud i alt':'Tilbud i alt','Ordinær uddannelse i alt':'Ordinær uddannelse','Øvrige vejlednings- og opkvalificeringsforløb':'Øvrig vejl./opkval. (ØVO)','Ansættelse med løntilskud i alt':'Løntilskud','Virksomhedspraktik i alt':'Virksomhedspraktik','Nytteindsats i alt':'Nytteindsats'}; updates=defaultdict(dict)
 for r in rows:
...
def refresh_notices(d):
 rows=records('data/y25i05?mgroup.*=*&period.M=latest:120&hierarchy._nykom=/&format=json'); sec=d['sections']['notices']; merge_arrays(sec,{str(r['Periode']):{'total':num(r.get('Varslinger, antal personer'))} for r in rows}); kpi(sec,sec['total'])
...
def refresh_graduates(d):
 merge_akasse(d['sections']['graduateUnemployed'],records('data/y01dia02?mgroup.*=*&period.M=latest:120&hierarchy._nykom=/&hierarchy._akassedp=*&format=json'),'Antal dimittendledige fuldtidspersoner')
...
def refresh_sanctions(d):
 ...
 for r in rows:
  p=str(r['Periode']); name=r.get('A-kasse'); target=series if name=='A-kasse i alt' else by.get(AKASSE_NAMES.get(name))
  if target:
...
def main():
 ...
 now=datetime.now(ZoneInfo('Europe/Copenhagen')); months=['januar','februar','marts','april','maj','juni','juli','august','september','oktober','november','december']; data['meta']['versionDate']=f'{now.day}. {months[now.month-1]} {now.year}'; data['meta']['sourceFile']='Officielle API-kilder med tidligere Excel-data som fallback'
